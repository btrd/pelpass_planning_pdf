...
module Planning
  class PdfGenerator
    ...
    def generate_all
      @missions.each do |mission_name, day_entries|
        ...
        day_entries.each do |entry|
          generate_for_day(mission_dir, mission_name, entry)
        end
      end
    end
    ...
    def generate_for_day(mission_dir, mission_name, entry)
      day = entry[:day]
      ...
      Prawn::Document.generate(filename, page_size: "A4", page_layout: :landscape) do |pdf|
        ...
        grouped_by_email.each_with_index do |(email, tasks), index|
          ...
          pdf.text_box(
            ...
            at: [0, y], width: LEFT_MARGIN - 10, height: ROW_HEIGHT,
            valign: :center, align: :left, size: 10
          )
          ...
          tasks.each do |task|
            ...
            pdf.fill_color("000000")
            pdf.text_box(label, at: [bar_x + 2, y], width: bar_width - 4, height: ROW_HEIGHT,
