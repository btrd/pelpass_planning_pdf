...
class PlanningGenerator
  attr_reader :csv, :headers, :missions

  def initialize
    @csv = CSV.read(INPUT_CSV, headers: true, encoding: 'bom|utf-8')
    @headers = csv.headers # Entêtes du tableau CSV
  ...
  def clean
    ...
    if File.exist?(ZIP_NAME)
      puts "Removing existing #{ZIP_NAME}"
      FileUtils.rm_f(ZIP_NAME)
    end

    if Dir.exist?(OUTPUT_DIR)
      puts "Removing existing #{OUTPUT_DIR}"
  ...
  puts "Creating #{ZIP_NAME} from #{OUTPUT_DIR} using rubyzip..."

  ZipFileGenerator.new(OUTPUT_DIR, ZIP_NAME).write()
